          ...
          >
            {previous ? (
              <div>
                ...
                <Link
                  href={`/posts/${previous?.slug.replace(
                    /^\d{4}-\d{2}-\d{2}\//,
                    ''
                  )}`}
                  className='font-bold'
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  return {
    paths: mdxFiles.map(file => ({
      params: {
        slug: file.frontMatter.slug.replace(/^\d{4}-\d{2}-\d{2}\//, ''),
      },
    })),
    fallback: false,
  };
...
export const getStaticProps: GetStaticProps = async context => {
  ...
  const postIndex = mdxFiles.findIndex(
    p => p.frontMatter.slug.replace(/^\d{4}-\d{2}-\d{2}\//, '') === slug
  );
